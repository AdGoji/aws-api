...
(ns ^:skip-wiki adgoji.cognitect.aws.signers
  "Impl, don't call directly."
  (:require [clojure.string :as str]
            ; #?(:clj [cognitect.aws.client :as client])
            ; #?(:clj [cognitect.aws.service :as service])
            [adgoji.cognitect.aws.util :as util]
...
(defn- canonical-uri
  ...
  (let [encoded-path (-> uri
                         ...
                         (uri-path)
                         (uri-encode "/"))]
    (if #?(:clj (.isEmpty ^String encoded-path)
           :cljs (= encoded-path ""))
      "/"
      encoded-path)))
...
(defn string-to-sign
  [request auth-info]
  (let [bytes #?(:clj (.getBytes ^String (canonical-request request))
                 :cljs (canonical-request request))]
    (str/join "\n" ["AWS4-HMAC-SHA256"
...
(defn signing-key
  ...
  (-> #?(:clj (.getBytes (str "AWS4" secret-access-key) "UTF-8")
         :cljs (.getBytes (str "AWS4" secret-access-key) "UTF-8"))
...
(defn v4-sign-http-request
  ...
  (let [{:keys [:aws/access-key-id :aws/secret-access-key :aws/session-token]} credentials
        auth-info      {:access-key-id     access-key-id
                        ...
                        :service           service #_(or (service/signing-name service)
...
;; We cannot use the client - Disable
#_ (do
