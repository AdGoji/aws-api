...
(ns ^:skip-wiki adgoji.cognitect.aws.util
...
(defn sha-256
  ...
  #?(:clj
     ...
     :cljs
     (.digest (doto (Sha256.) (.update (goog.crypt/stringToByteArray data))))))
...
#?(:clj
   ...
   (defn hmac-sha-256 [key message]
     (let [decode goog.crypt/stringToByteArray
           hasher (goog.crypt.Sha256.)
           decoded (decode key)
           hmacer (goog.crypt.Hmac. hasher decoded)]
       (goog.crypt/byteArrayToHex (.getHmac hmacer (decode message))))))
